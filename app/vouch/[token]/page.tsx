...
export default async function VouchPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const candidate = await db.user.findFirst({ where: { id: candidateId, role: { in: ["CANDIDATE", "QUALIFIED"] } }, include: { profile: true, vouchesReceived: { where: { isActive: true }, select: { id: true } } } });
