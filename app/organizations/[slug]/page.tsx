...
import { getPublicOrganizations, getPublicProjects } from "@/lib/data/public";

type Props = { params: Promise<{ slug: string }> };
export async function generateMetadata({ params }: Props): Promise<Metadata> { const slug = (await params).slug; const organizations = await getPublicOrganizations(); return { title: organizations.find((item) => item.slug === slug)?.name ?? "Organización" }; }
export default async function OrganizationPage({ params }: Props) {
  const [organizations, projects] = await Promise.all([getPublicOrganizations(), getPublicProjects()]);
  const slug = (await params).slug;
  const org = organizations.find((item) => item.slug === slug); if (!org) notFound(); const orgProjects = projects.filter((p) => p.organizationSlug === org.slug); const index = organizations.indexOf(org);
